...
const char *AgilV8Engine = "com/agil/agilv8/AgilV8Engine";
static AgilV8App *mApp = nullptr;
jobject globalAsset = nullptr;

extern "C" JNIEXPORT void JNICALL
native_init(JNIEnv *env, jobject instance, jobject javaAssetManager) {
    ALOGD("native_init")
    globalAsset = env->NewGlobalRef(javaAssetManager);
    mApp = new AgilV8App(env, javaAssetManager);
}

extern "C" JNIEXPORT void JNICALL
native_execute_js(JNIEnv *env, jobject instance, jstring path, jstring moduleName) {
    ALOGD("native_execute_js")
    auto pathStr = env->GetStringUTFChars(path, nullptr);
    auto nameStr = env->GetStringUTFChars(moduleName, nullptr);
    if (mApp != nullptr) {
        mApp->executeScript(pathStr, nameStr);
    }
    env->ReleaseStringUTFChars(path, pathStr);
    env->ReleaseStringUTFChars(moduleName, nameStr);
}
...
native_create(JNIEnv *env, jobject instance, jobject javaSurface) {
    ALOGD("native_create")
    if (mApp != nullptr) {
        mApp->create(ANativeWindow_fromSurface(env, javaSurface));
    }
}
...
native_release(JNIEnv *env, jobject instance, jlong time) {
    ...
    env->DeleteGlobalRef(globalAsset);
    delete mApp;
}

static JNINativeMethod g_RenderMethods[] = {
        {"nativeInit",      "(Landroid/content/res/AssetManager;)V",   (void *) native_init},
        {"nativeExecuteJS", "(Ljava/lang/String;Ljava/lang/String;)V", (void *) native_execute_js},
...
extern "C" jint JNI_OnLoad(JavaVM *jvm, void *p) {
    ...
    RegisterNativeMethods(env, AgilV8Engine, g_RenderMethods, std::size(g_RenderMethods));
...
extern "C" void JNI_OnUnload(JavaVM *jvm, void *p) {
    ...
    UnRegisterNativeMethods(env, AgilV8Engine);
}